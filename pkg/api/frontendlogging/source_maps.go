...
func (store *SourceMapStore) guessSourceMapLocation(sourceURL string) (*sourceMapLocation, error) {
	...
	if strings.HasPrefix(u.Path, "/public/build/") || (store.cfg.CDNRootURL != nil &&
		...
		for _, route := range store.pluginRoutes(context.TODO()) {
...
func (store *SourceMapStore) pluginRoutes(ctx context.Context) []*plugins.StaticRoute {
	staticRoutes := make([]*plugins.StaticRoute, 0)

	for _, p := range store.pluginStore.Plugins(ctx) {
