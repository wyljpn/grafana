...
export const VariableQueryEditor = ({ query, datasource, onChange }: Props) => {
  ...
  const { region, namespace, metricName, dimensionKey, valueDimensions, ec2Dimensions } = parsedQuery;
  ...
  const ec2DimensionKeys = useDimensionKeys(datasource, region, namespace, metricName, ec2Dimensions ?? {});
      ...
      {parsedQuery.queryType === VariableQueryType.EC2InstanceAttributes && (
        <>
          ...
          <InlineField
            label="Filters"
            labelWidth={20}
            tooltip='A JSON object representing dimensions/tags and the values to filter on. To filter on a tag, enter "tag:<tagName>" in the key field'
          >
            <Dimensions
              query={{ ...parsedQuery, dimensions: parsedQuery.ec2Dimensions }}
              onChange={(dimensions) => {
                onChange({ ...parsedQuery, ec2Dimensions: dimensions });
              }}
              dimensionKeys={ec2DimensionKeys}
              disableExpressions={true}
              datasource={datasource}
              multiValue={true}
            />
